...
$forums_info = $db->query('SELECT f.num_topics, f.num_posts, f.parent_forum_id, f.last_post_id, f.last_poster, f.last_post, f.id, f.forum_name, f.last_topic, us.username, us.id AS last_poster_id FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') LEFT JOIN '.$db->prefix.'users AS us ON (UPPER(us.username)=UPPER(f.last_poster)) WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.parent_forum_id <> 0 ORDER BY f.disp_position') or error('Unable to fetch subforum list', __FILE__, __LINE__, $db->error());
...
$result = $db->query('SELECT c.id AS cid, c.cat_name, c.cat_img, f.id AS fid, f.forum_name, f.forum_desc, f.redirect_url, f.moderators, f.num_topics, f.num_posts, f.last_post, f.last_post_id, f.last_poster, f.last_topic, f.parent_forum_id, us.username, us.id AS last_poster_id FROM '.$db->prefix.'categories AS c INNER JOIN '.$db->prefix.'forums AS f ON c.id=f.cat_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') LEFT JOIN '.$db->prefix.'users AS us ON (UPPER(us.username)=UPPER(f.last_poster)) WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND (f.parent_forum_id IS NULL OR f.parent_forum_id=0) ORDER BY c.disp_position, c.id, f.disp_position', true) or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());

$cur_category = 0;
$cat_count = 0;
$forum_count = 0;
$last_title_length = 27;
...
<div id="idx<?php echo $cat_count ?>" class="blocktable">
	<div class="box">
		<div class="inbox">
			<?php 
				if (isset($cur_forum['cat_img']) && !empty($cur_forum['cat_img'])) {
					$image_dir = 'img/'.pun_htmlspecialchars($cur_forum['cat_img']);

					list($width, $height) = getimagesize($image_dir);
			?>
				<div style="background: url('<?php echo $image_dir; ?>') repeat-x; height: <?php echo $height; ?>px;"></div>
			<?php } ?>
...
<?php
		...
		if (isset($sfdb[$cur_forum['fid']]))
		{
			foreach ($sfdb[$cur_forum['fid']] as $cur_subforum)
			{
				...
				if ($cur_forum['last_post'] < $cur_subforum['last_post'])
				{
					...
					$cur_forum['last_poster_id'] = $cur_subforum['last_poster_id'];
	...
	{
		$oldForumTopic = pun_htmlspecialchars($cur_forum['last_topic']);
		if (pun_strlen($cur_forum['last_topic']) > $last_title_length)
			$cur_forum['last_topic'] = utf8_substr($cur_forum['last_topic'], 0, $last_title_length).'...';

		$last_post = '<div class="avatar_class" style="background-image: url(\''. generate_avatar_markup_raw($cur_forum['last_poster_id']) .'\');"></div><span class="display-forum-info"><a href="viewtopic.php?pid='.$cur_forum['last_post_id'].'#p'.$cur_forum['last_post_id'].'" title="'.$oldForumTopic.'">'.pun_htmlspecialchars($cur_forum['last_topic']).'</a><br />'.format_time($cur_forum['last_post']).'<br />'.$lang_common['by'].' <a href="profile.php?id='.$cur_forum['last_poster_id'].'">'.pun_htmlspecialchars($cur_forum['last_poster']).'</a></span>';
	}
