...
if ($pid)
{
	...
	$result = $db->query('SELECT COUNT(id) FROM '.$db->prefix.'posts WHERE topic_id='.$id.' AND posted<'.$posted) or error('Unable to fetch post IDs', __FILE__, __LINE__, $db->error());
...
$post_ids = array();
for ($i = 0;$cur_post = $db->fetch_assoc($result, $i);$i++)
...
$result = $db->query('SELECT u.*, u.disp_name AS disp_name, p.id, p.poster AS username, p.poster_id, p.poster_ip, p.poster_email, p.message, p.hide_smilies, p.posted, p.edited, p.edited_by, g.g_id, g.g_user_title, o.user_id AS is_online FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'users AS u ON u.id=p.poster_id INNER JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id LEFT JOIN '.$db->prefix.'online AS o ON (o.user_id=u.id AND o.user_id!=1 AND o.idle=0) WHERE p.id IN ('.implode(',', $post_ids).') ORDER BY p.id', true) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
while ($cur_post = $db->fetch_assoc($result))
{
	...
	if ($cur_post['poster_id'] > 1)
	{
		...
		// Use Display Name or Username to display at posts.
		if (isset($cur_post['disp_name']) && !empty($cur_post['disp_name'])) {
			if ($pun_user['g_view_users'] == '1')
				$username = '<a href="profile.php?id='.$cur_post['poster_id'].'">'.pun_htmlspecialchars($cur_post['disp_name']).'</a>';
			else
				$username = pun_htmlspecialchars($cur_post['disp_name']);
		} else {
			if ($pun_user['g_view_users'] == '1')
		...
		foreach ($pun_bans as $cur_ban) {
			if ($cur_ban['username'] != '' && utf8_strtolower($cur_post['username']) == utf8_strtolower($cur_ban['username'])) {
		...
		if ($is_banned && $pun_user['username'] != $cur_post['username'])
			$user_title = '<div class="banned">'.$user_title.'</div><center><img src="img/youtried.png" width="128" height="128" /></center>';

		if (!$is_banned) {
			// Format the online indicator
			$is_online = ($cur_post['is_online'] == $cur_post['poster_id']) ? '<strong><img src="img/icons/status_online.png" title="'.pun_htmlspecialchars($cur_post['username']).' is Online" alt="Online" /></strong>' : '<span><img src="img/icons/status_offline.png" title="'.pun_htmlspecialchars($cur_post['username']).' is Offline" alt="Offline" /></span>';
			...
			//Handle the display of awards
			$user_info[] = show_user_awards($cur_post);
			$user_info[] = '<dd style="margin-bottom: 12px;"></dd>';

			$user_info[] = '<center>'. show_social_info($cur_post) .'</center>';
			$user_info[] = '<dd style="margin-bottom: 12px;"></dd>';
			...
			if ($pun_config['o_show_user_info'] == '1')
			{
				...
				$user_info[] = '<dd><span>Rocks: '.forum_number_format($cur_post['rocks']).'</span></dd></br>';
				...
				if (!$pun_user['is_guest'])
					$user_contacts[] = '<span class="pm"><a href="message_send.php?id='.$cur_post['poster_id'].'">PM</a></span>';
			}
		}
	...
	if (!$is_admmod)
	{
		...
		if ($cur_topic['closed'] == '0')
		{
			...
			if (($cur_topic['post_replies'] == '' && $pun_user['g_post_replies'] == '1') || $cur_topic['post_replies'] == '1') {
				$post_actions[] = '<li class="postquickquote"><span><a onmousedown="get_quote_text();" onclick="Quote(\''.pun_htmlspecialchars($cur_post['username']).'\', \''.pun_htmlspecialchars($cur_post['message']).'\'); return false;" href="post.php?tid='.$id.'&amp;qid='.$cur_post['id'].'">'.$lang_easy_bbcode['Quick quote'].'</a></span></li>';	
	...
	$cur_post['message'] = parse_message($cur_post['message'], $cur_post['hide_smilies'], $can_censor);
...
<?php if ($signature != '') echo "\t\t\t\t\t".'<div class="postsignature postmsg">'.$signature.'</div>'."\n"; ?>
				</div>
			</div>
		</div>
		<div class="inbox">
...
?>
					...
					<textarea name="req_message" rows="10" cols="75" tabindex="<?php echo $cur_index++ ?>"></textarea>
					</label>
						<ul class="bblinks">
