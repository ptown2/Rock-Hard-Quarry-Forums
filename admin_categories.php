...
if (isset($_POST['add_cat']))
{
	...
	$new_cat_img = isset($_POST['new_cat_img']) ? pun_trim($_POST['new_cat_img']) : null;
	if ($new_cat_name == '')
		message($lang_admin_categories['Must enter name message']);

	$db->query('INSERT INTO '.$db->prefix.'categories (cat_name, cat_img) VALUES(\''.$db->escape($new_cat_name).'\', \''.$db->escape($new_cat_img).'\')') or error('Unable to create category', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['update'])) // Change position and name of the categories
{
	...
	foreach ($categories as $cat_id => $cur_cat)
	{
		...
		$cur_cat['image'] = isset($cur_cat['image']) ? pun_trim($cur_cat['image']) : null;
		...
		$db->query('UPDATE '.$db->prefix.'categories SET cat_name=\''.$db->escape($cur_cat['name']).'\', cat_img=\''.$db->escape($cur_cat['image']).'\', disp_position='.$cur_cat['order'].' WHERE id='.intval($cat_id)) or error('Unable to update category', __FILE__, __LINE__, $db->error());
	}
...
$result = $db->query('SELECT id, cat_name, cat_img, disp_position FROM '.$db->prefix.'categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
						...
						<div class="infldset">
							...
							<thead>
								<tr>
									...
									<th class="tcl" scope="col"><?php echo $lang_admin_categories['Category image label'] ?></th>
...
?>
								<tr>
									...
									<td class="tcl"><input type="text" name="cat[<?php echo $cur_cat['id'] ?>][image]" value="<?php echo pun_htmlspecialchars($cur_cat['cat_img']) ?>" size="45" maxlength="80" /></td>
