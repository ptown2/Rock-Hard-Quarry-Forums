...
//
// Generate the donation cache PHP script
//
function generate_donations_cache()
{
	global $db;

	// Get the donations list from the DB
	$result = $db->query('SELECT * FROM '.$db->prefix.'donations', true) or error('Unable to fetch donations list', __FILE__, __LINE__, $db->error());

	$output = array();
	while ($cur_donations = $db->fetch_assoc($result))
		$output[] = $cur_donations;

	// Output donations list as PHP code
	$fh = @fopen(FORUM_CACHE_DIR.'cache_donations.php', 'wb');
	if (!$fh)
		error('Unable to write donations cache file to cache directory. Please make sure PHP has write access to the directory \''.pun_htmlspecialchars(FORUM_CACHE_DIR).'\'', __FILE__, __LINE__);

	fwrite($fh, '<?php'."\n\n".'define(\'PUN_DONATIONS_LOADED\', 1);'."\n\n".'$pun_donations = '.var_export($output, true).';'."\n\n".'?>');

	fclose($fh);

	if (function_exists('apc_delete_file'))
		@apc_delete_file(FORUM_CACHE_DIR.'cache_donations.php');
}

//
// Generate the 'stocks' from stock market simulator cache PHP script
//
function generate_stock_cache()
{
	global $db;

	// Get the stocks list from the DB
	$result = $db->query('SELECT * FROM '.$db->prefix.'stock_share', true) or error('Unable to fetch stock share list', __FILE__, __LINE__, $db->error());

	$output = array();
	while ($stocks = $db->fetch_assoc($result))
		$output[] = $stocks;

	// Output stocks list as PHP code
	$fh = @fopen(FORUM_CACHE_DIR.'cache_stocks.php', 'wb');
	if (!$fh)
		error('Unable to write stocks cache file to cache directory. Please make sure PHP has write access to the directory \''.pun_htmlspecialchars(FORUM_CACHE_DIR).'\'', __FILE__, __LINE__);

	fwrite($fh, '<?php'."\n\n".'define(\'PUN_STOCKS_LOADED\', 1);'."\n\n".'$pun_stocks = '.var_export($output, true).';'."\n\n".'?>');

	fclose($fh);

	if (function_exists('apc_delete_file'))
		@apc_delete_file(FORUM_CACHE_DIR.'cache_stocks.php');
}

//
// Generate the 'arcade' games cache PHP script
//
function generate_games_cache()
{
	global $db;

	// Get the games list from the DB
	$result = $db->query('SELECT * FROM '.$db->prefix.'game_list', true) or error('Unable to fetch game list', __FILE__, __LINE__, $db->error());

	$output = array();
	while ($game_info = $db->fetch_assoc($result))
		$output[] = $game_info;

	// Output games list as PHP code
	$fh = @fopen(FORUM_CACHE_DIR.'cache_games.php', 'wb');
	if (!$fh)
		error('Unable to write games list cache file to cache directory. Please make sure PHP has write access to the directory \''.pun_htmlspecialchars(FORUM_CACHE_DIR).'\'', __FILE__, __LINE__);

	fwrite($fh, '<?php'."\n\n".'define(\'PUN_GAMES_LOADED\', 1);'."\n\n".'$pun_games = '.var_export($output, true).';'."\n\n".'?>');

	fclose($fh);

	if (function_exists('apc_delete_file'))
		@apc_delete_file(FORUM_CACHE_DIR.'cache_games.php');
}
