...
// Keep the OpenID Data Session available.
session_start();
$ob_username = isset($_SESSION['username']) ? pun_trim($_SESSION['username']) : null;
$ob_email = isset($_SESSION['email']) ? strtolower(pun_trim($_SESSION['email'])) : null;
$openid = isset($_SESSION['openid']) ? pun_trim($_SESSION['openid']) : null;
$key = isset($_SESSION['key']) ? pun_trim($_SESSION['key']) : null;
...
if (isset($_POST['form_sent']))
{
	...
	$result = $db->query('SELECT 1 FROM '.$db->prefix.'users WHERE registration_ip=\''.$db->escape(get_remote_address()).'\' AND registered>'.(time() - 7200)) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());

	if ($db->num_rows($result))
		message($lang_register['Registration flood']);

	$username = isset($_POST['req_honeypot']) ? pun_trim($_POST['req_honeypot']) : $ob_username;
	$email1 = isset($_POST['req_email1']) ? strtolower(pun_trim($_POST['req_email1'])) : $ob_email;

	if ($pun_config['o_regs_verify'] == '1')
	{
		$email2 = isset($_POST['req_email2']) ? strtolower(pun_trim($_POST['req_email2'])) : $ob_email;

		$password1 = random_pass(20);
		$password2 = $password1;
	}
	else
	{
		if (!empty($openid) && !empty($key)) {
			$password1 = random_pass(20);
			$password2 = $password1;
		} else {
	...
	if (empty($openid) && empty($key)) {
		...
		// Recapcha Checking
		require_once 'include/recaptcha/vendor/autoload.php';

		$recaptcha = new \ReCaptcha\ReCaptcha(CAPTCHA_SECRET);
		$resp = $recaptcha->verify($_POST['g-recaptcha-response'], $_SERVER['REMOTE_ADDR']);

		if (!$resp->isSuccess())  
			$errors[] = 'Wrong/Failed reCaptcha Verification! Try again!';
	}
	...
	if (empty($errors))
	{
		...
		// Set the OpenID Queries
		$open_sql = '';
		$open_sql2 = '';
		if ($openid == 'steam') {
			$open_sql = 'steam64, ';
			$open_sql2 = "'".$db->escape($key)."', ";
		} elseif ($openid == 'facebook') {
			$open_sql = 'facebook_id, ';
			$open_sql2 = "'".$db->escape($key)."', ";
		} elseif ($openid == 'twitter') {
			$open_sql = 'twitter_id, ';
			$open_sql2 = "'".$db->escape($key)."', ";
		...
		// Add the user
		$db->query('INSERT INTO '.$db->prefix.'users ('.$open_sql.'username, group_id, password, email, email_setting, timezone, dst, language, style, registered, registration_ip, last_visit) VALUES('.$open_sql2.' \''.$db->escape($username).'\', '.$intial_group_id.', \''.$password_hash.'\', \''.$db->escape($email1).'\', '.$email_setting.', '.$timezone.' , '.$dst.', \''.$db->escape($language).'\', \''.$pun_config['o_default_style'].'\', '.$now.', \''.$db->escape(get_remote_address()).'\', '.$now.')') or error('Unable to create user', __FILE__, __LINE__, $db->error());
		$new_uid = $db->insert_id();

		// Is the ID a Prime User ID?
		if (isPrime($new_uid))
			$db->query('UPDATE '.$db->prefix.'users SET a_prime=1 WHERE id='. $new_uid); //or error('Unable to update award', __FILE__, __LINE__, $db->error());
		...
		if ($pun_config['o_regs_verify'] == '1')
		{
			...
			if (!empty($openid) && !empty($key)) {
				generate_user_pm($new_uid, $password1, $now, get_remote_address(), true);
				redirect('login.php', 'Registration Complete!</br>Your account has been successfully linked and can be used any time.</br>Do notice that your password will be sent (if Google) but you cannot use it to log-in with the legacy mode.</br></br>Redirecting you back to the login page.');
			} else {
				generate_user_pm($new_uid, $password1, $now, get_remote_address(), false);
				redirect('login.php', $lang_register['Reg email'].' <a href="mailto:'.$pun_config['o_admin_email'].'">'.$pun_config['o_admin_email'].'</a>.', true);
			}
		}
		...
		if (!empty($openid) && !empty($key)) {
			generate_user_pm($new_uid, $password1, $now, get_remote_address(), true);
			redirect('index.php', 'Registration Complete!</br>Your account has been successfully linked and can be used any time.');
		} else {
			generate_user_pm($new_uid, $password1, $now, get_remote_address(), false);
...
<div id="regform" class="blockform">
	...
	<div class="box">
		<form id="register" method="post" action="register.php?action=register" onsubmit="this.register.disabled=true;if(process_form(this)){return true;}else{this.register.disabled=false;return false;}">
			<div class="inform">
				<?php if (empty($ob_username) && empty($ob_email) && empty($openid) && empty($key)) { ?>
				<div class="forminfo">
					<h3>OpenID Registration Method</h3>
					<p>You can use either one of those options to register with the OpenID system or you can use the legacy method as well.</p>
					<br />
					<a href="login.php?openid=google"><img class="login_button" src="img/logins/login_google.png"></a>
					<a href="login.php?openid=steam"><img class="login_button" src="img/logins/login_steam.png"></a>
				</div>
				<?php } ?>
				<div class="forminfo">
				...
				<fieldset>
					...
					<div class="infldset">
						...
						<label class="required usernamefield" style="display: none;"><strong><?php echo $lang_register['If human'] ?></strong><br /><input type="text" name="req_username" value="" size="25" maxlength="25" /><br /></label>
						<label class="required"><strong><?php echo $lang_common['Username'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /><input type="text" name="req_honeypot" value="<?php echo (isset($_POST['req_honeypot']) ? pun_htmlspecialchars($_POST['req_honeypot']) : pun_htmlspecialchars($ob_username)); ?>" size="25" maxlength="25" /><br /></label>
					</div>
				</fieldset>
			</div>
<?php 
...
if ($pun_config['o_regs_verify'] == '0'): ?>			<div class="inform">
				<fieldset>
