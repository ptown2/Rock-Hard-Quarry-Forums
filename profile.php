...
require PUN_ROOT.'include/openid.php';
...
// OpenID System for profile.php
require PUN_ROOT.'openid_profile.php';
...
// Load the awards language file
if (file_exists(PUN_ROOT.'/lang/'.$pun_user['language'].'/admin_awards.php')) 
	require PUN_ROOT.'/lang/'.$pun_user['language'].'/admin_awards.php';
else 
	require PUN_ROOT.'/lang/English/admin_awards.php';

//
// EASY AVATAR START
//
$oldsize = $pun_config['o_avatars_size'];
$oldwidth = $pun_config['o_avatars_width'];
$oldheight = $pun_config['o_avatars_height'];

if (($pun_user['g_id'] <= PUN_MOD) || ($pun_user['g_id'] == PUN_GLMOD)) {
	$pun_config['o_avatars_size'] = 1048578;
	$pun_config['o_avatars_width'] = 200;
	$pun_config['o_avatars_height'] = 2048;
} elseif ($pun_user['g_id'] == PUN_DONATOR) {
	$pun_config['o_avatars_size'] = ( $pun_config['o_avatars_size'] * 2 );
	$pun_config['o_avatars_width'] = 200;
	$pun_config['o_avatars_height'] = 400;
}

// Altering the non-avatar accounted ones.
if ($id == 0) {
	$pun_config['o_avatars_size'] = $oldsize;
	$pun_config['o_avatars_width'] = $oldwidth;
	$pun_config['o_avatars_height'] = $oldheight;
}
...
function reduce_image($image_to_reduce) {
	global $pun_config, $lang_EZavatar, $width, $height, $size, $extension;
	...
	if ($type == IMAGETYPE_GIF || $type == IMAGETYPE_PNG) {
		$trnprt_indx = imagecolortransparent($image_to_reduce);

		if ($trnprt_indx >= 0) {
			$trnprt_color = imagecolorsforindex($image_to_reduce, $trnprt_indx);
			$trnprt_indx = imagecolorallocate($reduced_image, $trnprt_color['red'], $trnprt_color['green'], $trnprt_color['blue']);
			imagefill($reduced_image, 0, 0, $trnprt_indx);
			imagecolortransparent($reduced_image, $trnprt_indx);
		} elseif ($type == IMAGETYPE_PNG) {
			imagealphablending($reduced_image, false);
			$color = imagecolorallocatealpha($reduced_image, 0, 0, 0, 127);
			imagefill($reduced_image, 0, 0, $color);
			imagesavealpha($reduced_image, true);
		}
	}
...
else if ($action == 'upload_avatar' || $action == 'upload_avatar2')
{
	...
	//if (($pun_user['id'] != $id || $id != 0) && !$pun_user['is_admmod'])
...
elseif (isset($_POST['update_awards']))
{
	if ($pun_user['g_id'] != PUN_GLMOD && $pun_user['g_id'] != PUN_ADMIN)
		message($lang_common['No permission'], false, '403 Forbidden');

	confirm_referrer('profile.php');

	$form = array(
		'a_prime' => (isset($_POST['prime'])) ? '1' : '0',
		'a_loyal' => (isset($_POST['loyal'])) ? '1' : '0',
		'a_thoughtful' => (isset($_POST['thoughtful'])) ? '1' : '0',
		'a_creative' => (isset($_POST['creative'])) ? '1' : '0',
		'a_friendly' => (isset($_POST['friendly'])) ? '1' : '0',
		'a_reporter' => (isset($_POST['reporter'])) ? '1' : '0',
		'a_cutie' => (isset($_POST['cutie'])) ? '1' : '0',
		'a_special' => (isset($_POST['special'])) ? '1' : '0',
		'a_forumban' => (isset($_POST['forumban'])) ? '1' : '0',
		'a_shitpost' => (isset($_POST['shitpost'])) ? '1' : '0',
		'a_anime' => (isset($_POST['anime'])) ? '1' : '0',	
		'a_abuse' => (isset($_POST['abuse'])) ? '1' : '0',	
	);

	// Prevent HTML Faking to bypass the cutie award.
	if ($pun_user['id'] != '2')
		$form['a_cutie'] = '0';
	...
	redirect('profile.php?section=admin&id='.$id, $lang_admin_awards['Edited user awards']);
}
...
else if (isset($_POST['form_sent']))
{
	...
	switch ($section)
	{
		...
		{
			$form = array(
				'allow_legacy'	=> isset($_POST['form']['allow_legacy']) ? '1' : '0',
		...
		case 'personal':
		{
			$form = array(
				...
				'disp_name'		=> pun_trim($_POST['form']['disp_name']),
				'url'			=> pun_trim($_POST['form']['url']),
				'location'		=> pun_trim($_POST['form']['location']),
			);

			//if ($user['donated'] < '10.00') {
			//	$form['disp_name'] = null;
			//}

			check_dispname($form['disp_name']);
			...
			else if ($pun_user['g_set_title'] == '1')
			{
				...
				if ($form['title'] != '')
				{
					...
					$forbidden = array('member', 'moderator', 'global moderator', 'administrator', 'developer', 'banned', 'guest', utf8_strtolower($lang_common['Member']), utf8_strtolower($lang_common['Moderator']), utf8_strtolower($lang_common['Administrator']), utf8_strtolower($lang_common['Banned']), utf8_strtolower($lang_common['Guest']));
	...
	$user['g_moderator'] == '1'))))																// or the user is another mod
{
	...
	if ($user['disp_name'] != '')
	{
		$user_personal[] = '<dt>'.$lang_profile['Displayname'].'</dt>';
		$user_personal[] = '<dd>'.pun_htmlspecialchars(($pun_config['o_censoring'] == '1') ? censor_words($user['disp_name']) : $user['disp_name']).'</dd>';
	}
	...
	if ($user['facebook'] != '')
	{
		$user_social[] = '<dt>'.$lang_profile['Facebook'].'</dt>';
		$user_social[] = '<dd>'.pun_htmlspecialchars(($pun_config['o_censoring'] == '1') ? censor_words($user['facebook']) : $user['facebook']).'</dd>';
	}

	if ($user['twitter'] != '')
	{
		$user_social[] = '<dt>'.$lang_profile['Twitter'].'</dt>';
		$user_social[] = '<dd>'.pun_htmlspecialchars(($pun_config['o_censoring'] == '1') ? censor_words($user['twitter']) : $user['twitter']).'</dd>';
	}

	if ($user['steam'] != '')
	{
		$user_social[] = '<dt>'.$lang_profile['Steam'].'</dt>';
		$user_social[] = '<dd>'.pun_htmlspecialchars(($pun_config['o_censoring'] == '1') ? censor_words($user['steam']) : $user['steam']).'</dd>';
	}

	if ($user['skype'] != '')
	{
		$user_social[] = '<dt>'.$lang_profile['Skype'].'</dt>';
		$user_social[] = '<dd>'.pun_htmlspecialchars(($pun_config['o_censoring'] == '1') ? censor_words($user['skype']) : $user['skype']).'</dd>';
	}

	if ($user['yahoo'] != '')
	{
		$user_social[] = '<dt>'.$lang_profile['Yahoo'].'</dt>';
		$user_social[] = '<dd>'.pun_htmlspecialchars(($pun_config['o_censoring'] == '1') ? censor_words($user['yahoo']) : $user['yahoo']).'</dd>';
	}

	if ($user['youtube'] != '')
	{
		$user_social[] = '<dt>'.$lang_profile['YouTube'].'</dt>';
		$user_social[] = '<dd>'.pun_htmlspecialchars(($pun_config['o_censoring'] == '1') ? censor_words($user['youtube']) : $user['youtube']).'</dd>';
	}
...
<?php endif; if (!empty($user_social)): ?>			<div class="inform">
				<fieldset>
				<legend><?php echo $lang_profile['Section social'] ?></legend>
					<div class="infldset">
						<dl>
							<?php echo implode("\n\t\t\t\t\t\t\t", $user_social)."\n" ?>
						</dl>
				...
				<legend><?php echo $lang_profile['User activity'] ?></legend>
					<div class="infldset">
						<dl>
							...
							<dd>
							<?php
								$awardstxt = show_user_awards($user);

								echo (isset($awardstxt) && $awardstxt != '') ? $awardstxt : 'No awards earned.'; 
							?>
							</dd>
						</dl>
						<div class="clearer"></div>
	...
	<div class="blockform">
		...
		<div class="box">
			<form id="profile1" method="post" action="profile.php?section=essentials&amp;id=<?php echo $id ?>" onsubmit="return process_form(this)">
				...
				<?php if ($pun_user['id'] == $user['id']) { ?>
				<div class="inform">
					<fieldset>
						<legend>OpenID Account Linkage</legend>
						<div class="infldset">
							<?php if (empty($user['google_id'])) { ?> <a href="?id=<?php echo $id; ?>&openid=google"><img class="login_button" src="img/logins/login_google.png"></a> <?php } else { echo '<p><b style="font-size: 14px;">You are already linked with your Google Account!</b></p>'; } ?>
							<?php if (empty($user['steam64'])) { ?> <a href="?id=<?php echo $id; ?>&openid=steam"><img class="login_button" src="img/logins/login_steam.png"></a> <?php } else { echo '<p><b style="font-size: 14px;">You are already linked with your Steam Account!</b></p>'; } ?>

							</br>
							<?php if (!empty($user['steam64']) || !empty($user['steam64'])) { ?>
							<div class="rbox">
								<label><input type="checkbox" name="form[allow_legacy]" value="1"<?php if ($user['allow_legacy'] == '1') echo ' checked="checked"' ?> />Allow this profile to use the legacy log-in method (NOT RECOMMENDED)<br /></label>
							</div>
							<?php } ?>
						</div>
					</fieldset>
...
<?php require PUN_ROOT.'include/warnings/warnings_profile.php'; ?>
...
<?php if ($user['donated'] >= '10.00'):	?>
							<label><?php echo $lang_profile['Displayname'] ?><br /><input type="text" name="form[disp_name]" value="<?php echo pun_htmlspecialchars($user['disp_name']) ?>" size="40" maxlength="40" /><br /></label>
<?php endif; ?>
		...
		<div class="box">
			<form id="profile3" method="post" action="profile.php?section=messaging&amp;id=<?php echo $id ?>">
				<div class="inform">
					...
					<br />
					<fieldset>
						<legend>Enter your social contact details</legend>
						<div class="infldset">
							<input type="hidden" name="form_sent" value="1" />
							<label><?php echo $lang_profile['Facebook'] ?><br /><input id="facebook" type="text" name="form[facebook]" value="<?php echo pun_htmlspecialchars($user['facebook']) ?>" size="30" maxlength="64" /><br /></label>
							<label><?php echo $lang_profile['Twitter'] ?><br /><input id="twitter" type="text" name="form[twitter]" value="<?php echo pun_htmlspecialchars($user['twitter']) ?>" size="30" maxlength="64" /><br /></label>
							<label><?php echo $lang_profile['Steam'] ?><br /><input id="steam" type="text" name="form[steam]" value="<?php echo pun_htmlspecialchars($user['steam']) ?>" size="30" maxlength="64" /><br /></label>
							<label><?php echo $lang_profile['Skype'] ?><br /><input id="skype" type="text" name="form[skype]" value="<?php echo pun_htmlspecialchars($user['skype']) ?>" size="30" maxlength="64" /><br /></label>
							<label><?php echo $lang_profile['Yahoo'] ?><br /><input id="yahoo" type="text" name="form[yahoo]" value="<?php echo pun_htmlspecialchars($user['yahoo']) ?>" size="30" maxlength="64" /><br /></label>
							<label><?php echo $lang_profile['YouTube'] ?><br /><input id="youtube" type="text" name="form[youtube]" value="<?php echo pun_htmlspecialchars($user['youtube']) ?>" size="30" maxlength="64" /><br /></label>
						</div>
					</fieldset>
				</div>
				<p class="buttons"><input type="submit" name="update" value="<?php echo $lang_common['Submit'] ?>" /> <?php echo $lang_profile['Instructions'] ?></p>
...
?>
<?php if (($pun_user['g_id'] == PUN_GLMOD || $pun_user['g_id'] == PUN_ADMIN) && ($pun_user['id'] != $user['id'] && $pun_user['g_id'] != $user['g_id']) || $pun_user['g_id'] == PUN_ADMIN): ?>
			<div class="inform">
					<fieldset>
						<legend><?php echo $lang_admin_awards['Set awards'] ?></legend>
						<div class="infldset">
							<p><?php echo $lang_admin_awards['Awards info'] ?></p>
							<input type="hidden" name="form_sent_award" value="1" />

							<p><input type="checkbox" name="prime" value="prime" <?php if ($user['a_prime'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/prime.png" title="Prime Number" alt="Prime Number" /> Prime Number</p>
							<p><input type="checkbox" name="loyal" value="loyal" <?php if ($user['a_loyal'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/loyal.png" title="Loyal" alt="Loyal" /> Loyal</p>
							<p><input type="checkbox" name="thoughtful" value="thoughtful" <?php if ($user['a_thoughtful'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/thoughtful.png" title="Thoughtful" alt="Thoughtful" /> Thoughtful</p>
							<p><input type="checkbox" name="creative" value="creative" <?php if ($user['a_creative'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/creative.png" title="Creative" alt="Creative" /> Creative</p>
							<p><input type="checkbox" name="friendly" value="friendly" <?php if ($user['a_friendly'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/friendly.png" title="Friendly" alt="Friendly" /> Friendly</p>
							<p><input type="checkbox" name="reporter" value="reporter" <?php if ($user['a_reporter'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/reporter.png" title="Reporter" alt="Reporter" /> Reporter</p>
<?php if ($pun_user['id'] == '2'): ?>
							<p><input type="checkbox" name="cutie" value="cutie" <?php if ($user['a_cutie'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/rainbow.png" title="Cutie" alt="Cutie" /> Cutie</p>
							<p><input type="checkbox" name="abuse" value="abuse" <?php if ($user['a_abuse'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/abuse.png" title="Admin Abused" alt="Admin Abused" /> Admin Abused</p>
<?php endif; ?>
							<p><input type="checkbox" name="anime" value="anime" <?php if ($user['a_anime'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/anime.png" title="Sempai Club" alt="Sempai Club" /> Sempai Club</p>
							<p><input type="checkbox" name="special" value="special" <?php if ($user['a_special'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/special.png" title="Special" alt="Special" /> Special</p>
							<p><input type="checkbox" name="forumban" value="forumban" <?php if ($user['a_forumban'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/hsnban.png" title="SN&D Ban" alt="SN&D Ban" /> Sensationalist News and Debates Ban</p>
							<p><input type="checkbox" name="shitpost" value="shitpost" <?php if ($user['a_shitpost'] == 1) { echo 'checked'; } ?> /> <img src="img/awards/shitposter.png" title="Shitposter" alt="Shitposter" /> Shitposter</p>

							<br class="clearb" />
							<input type="submit" name="update_awards" value="<?php echo $lang_admin_awards['Update awards'] ?>" />
						</div>
					</fieldset>			
			</div>
			<?php endif; ?>
