...
$sort_by = isset($_GET['sort_by']) && (in_array($_GET['sort_by'], array('username', 'registered', 'rocks')) || ($_GET['sort_by'] == 'num_posts' && $show_post_count)) ? $_GET['sort_by'] : 'username';
...
?>
						...
						<br /><select name="sort_by">
							...
							<option value="rocks"<?php if ($sort_by == 'rocks') echo ' selected="selected"' ?>>Amount of Rocks</option>
		...
		<div class="inbox">
			...
			<thead>
				<tr>
					...
					<th class="tc3" scope="col">Rocks</th>
...
if ($db->num_rows($result))
{
	...
	// Grab the users
	$result = $db->query('SELECT u.id, u.username, u.title, u.rocks, u.num_posts, u.registered, g.g_id, g.g_user_title FROM '.$db->prefix.'users AS u LEFT JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id WHERE u.id IN('.implode(',', $user_ids).') ORDER BY '.$sort_by.' '.$sort_dir.', u.id ASC') or error('Unable to fetch user list', __FILE__, __LINE__, $db->error());
...
?>
				<tr>
					...
					<td class="tc3"><?php echo forum_number_format($user_data['rocks']); ?></td>
