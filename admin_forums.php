...
else if (isset($_GET['edit_forum']))
{
	...
	if (isset($_POST['save']))
	{
		...
		$is_incr = intval($_POST['post_inc']);
		$is_cens = intval($_POST['can_cens']);
		...
		$db->query('UPDATE '.$db->prefix.'forums SET forum_name=\''.$db->escape($forum_name).'\', forum_desc='.$forum_desc.', redirect_url='.$redirect_url.', sort_by='.$sort_by.', post_inc='.$is_incr.', can_cens='.$is_cens.', cat_id='.$cat_id.', parent_forum_id='.$parent_forum_id.' WHERE id='.$forum_id) or error('Unable to update forum', __FILE__, __LINE__, $db->error());
	...
	// Fetch forum info
	$result = $db->query('SELECT id, forum_name, forum_desc, redirect_url, num_topics, sort_by, post_inc, can_cens, cat_id, parent_forum_id FROM '.$db->prefix.'forums WHERE id='.$forum_id) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
...
?>
								...
								<tr>
									<th scope="row">Increment post number to user?</th>
									<td><input type="checkbox" name="post_inc" value="1"<?php echo ($cur_forum['post_inc']) ? ' checked="checked"' : ''; ?> tabindex="5" /></td>
								</tr>
								<tr>
									<th scope="row">Can censor posts in any way?</th>
									<td><input type="checkbox" name="can_cens" value="1"<?php echo ($cur_forum['can_cens']) ? ' checked="checked"' : ''; ?> tabindex="6" /></td>
								</tr>
								<tr>
									<th scope="row"><?php echo $lang_admin_forums['Redirect label'] ?></th>
									<td><?php echo ($cur_forum['num_topics']) ? $lang_admin_forums['Redirect help'] : '<input type="text" name="redirect_url" size="45" maxlength="100" value="'.pun_htmlspecialchars($cur_forum['redirect_url']).'" tabindex="7" />'; ?></td>
								</tr>
								<tr>
