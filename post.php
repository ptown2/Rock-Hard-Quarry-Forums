...
if ($tid)
	$result = $db->query('SELECT f.id, f.post_inc, f.can_cens, f.forum_name, f.moderators, f.redirect_url, fp.post_replies, fp.post_topics, t.subject, t.closed, s.user_id AS is_subscribed FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') LEFT JOIN '.$db->prefix.'topic_subscriptions AS s ON (t.id=s.topic_id AND s.user_id='.$pun_user['id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$tid) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
else
	$result = $db->query('SELECT f.id, f.post_inc, f.can_cens, f.forum_name, f.moderators, f.redirect_url, fp.post_replies, fp.post_topics FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.id='.$fid) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
...
$can_censor = $cur_posting['can_cens'];
...
if ($tid && $can_censor == '1' && $pun_config['o_censoring'] == '1')
...
if (isset($_POST['form_sent']))
{
	...
	// Shitposter Badge
	if ($pun_user['a_shitpost'] == "1")
		$pun_user['g_post_flood'] = $pun_user['g_post_flood'] * 6;

	// Flood protection
	if (!isset($_POST['preview']) && $pun_user['last_post'] != '' && (time() - $pun_user['last_post']) < $pun_user['g_post_flood'])
		$errors[] = $lang_post['Flood start'].' '.$pun_user['g_post_flood'].' '.$lang_post['flood end'];

	// Banned from a specific section.
	if ($pun_user['a_forumban'] && $cur_posting['id'] == '18')
		$errors[] = 'You are blacklisted from posting in this section.';

	// BANNED FROM POSTING.
	foreach ($pun_bans as $cur_ban) {
		if ($cur_ban['username'] != '' && utf8_strtolower($pun_user['username']) == utf8_strtolower($cur_ban['username'])) {
			if ($cur_ban['ban_type'] == '2') {
				$errors[] = 'You have been <b>POST SUSPENDED</b> for the reason: '.(isset($cur_ban['reason']) ? '"'.$cur_ban['reason'].'"' : '"No reason specified"').'. '.$lang_common['Ban message 2'].' '.(isset($cur_ban['expire']) ? date("F j, Y, g:i a", $cur_ban['expire']) : '<b style="color: red;">NEVER</b>');
				break;
			}
		}
	}
	...
	if ($fid)
	{
		...
		if ($can_censor == '1' && $pun_config['o_censoring'] == '1')
			$censored_subject = pun_trim(censor_words($subject));

		if ($subject == '')
			$errors[] = $lang_post['No subject'];
		else if ($can_censor == '1' && $pun_config['o_censoring'] == '1' && $censored_subject == '')
	...
	if (empty($errors))
	{
		...
		else if ($can_censor == '1' && $pun_config['o_censoring'] == '1')
		{
	...
	// Ninja Posting
	/*if ($tid) {
		$stamptime = intval($_POST['stamp']);
		$postquery = $db->query('SELECT count(posted) FROM '.$db->prefix.'posts WHERE (posted > '.$stamptime.' AND topic_id='.$tid.')');
		$postresult = $db->fetch_assoc($postquery);

		if ($postresult['count(posted)'] == 1) {
			$errors[] = $lang_post['New post'];
		}
		else if ($postresult['count(posted)'] > 1) {
			$errors[] = sprintf($lang_post['New posts'], $postresult['count(posted)']);
		}
	}*/

	// Ninja Posting
	/*if ($tid) {
		$stamp_time = intval($_POST['stamp']);
		$post_query = $db->query('SELECT count(posted) AS count_number FROM '.$db->prefix.'posts WHERE posted > '.$stamp_time.'');
		$post_result = $db->fetch_assoc($post_query);

		echo $post_result['count_number'];

		if ($post_result['count_number'] == 1) {
			$errors[] = $lang_post['New post'];
		}
		else if ($post_result['count_number'] > 1) {
			$errors[] = sprintf($lang_post['New posts'], $post_result['count_number']);
		}
	}*/
	...
	require PUN_ROOT.'include/mods/bumpsage_post.php';
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		...
		if ($tid)
		{
			if (!$pun_user['is_guest'])
			{
				...
				$result = $db->query('SELECT last_post, last_poster, last_post_id FROM '.$db->prefix.'topics WHERE id='.$new_tid) or error('Unable to fetch data of last post for topic', __FILE__, __LINE__, $db->error());
				$last_post = $db->fetch_assoc($result);

				$poster = $last_post['last_poster'];
				$id = $last_post['last_post_id'];
				$lasttime = $last_post['last_post'] + ( 60 * 60 * 2 );

				// If last poster is current poster
				if ((utf8_strtolower($poster) == utf8_strtolower($username)) && ($lasttime > $now))
				{
					$message = "\n\n\n".sprintf($lang_bumpsage['bump result'], gmdate('D, d M Y H:i:s', $now))."\n\n".$message;
					$edited_sql = ', edited='.$now.', edited_by=poster';
					...
					$db->query('UPDATE '.$db->prefix.'topics SET last_post='.$now.' WHERE id='.$new_tid) or error('Unable to update topics', __FILE__, __LINE__, $db->error());
			...
			if ($sage)
				$db->query('UPDATE '.$db->prefix.'topics SET num_replies=num_replies+1, last_post_id='.$new_pid.' WHERE id='.$tid) or error('Unable to update topic', __FILE__, __LINE__, $db->error());
			else
					...
					while ($cur_subscriber = $db->fetch_assoc($result))
					{
						...
						if (!isset($notification_emails[$cur_subscriber['language']]))
						{
							if (file_exists(PUN_ROOT.'lang/'.$cur_subscriber['language'].'/mail_templates/new_topic.tpl'))
							{
								...
								$mail_message = str_replace('<topic_subject>', ($can_censor == '1' && $pun_config['o_censoring'] == '1') ? $censored_subject : $subject, $mail_message);
								...
								$mail_message_full = str_replace('<topic_subject>', ($can_censor == '1' && $pun_config['o_censoring'] == '1') ? $censored_subject : $subject, $mail_message_full);
		...
		{
			$postinc = 1;
			if ($cur_posting['post_inc'] == '0') {
				$postinc = 0;
			}

			$db->query('UPDATE '.$db->prefix.'users SET num_posts=num_posts+'.$postinc.', last_post='.$now.' WHERE id='.$pun_user['id']) or error('Unable to update user', __FILE__, __LINE__, $db->error());
...
if ($fid): ?>
						<label class="required"><strong><?php echo $lang_common['Subject'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /><input class="longinput" type="text" name="req_subject" value="<?php if (isset($_POST['req_subject'])) echo pun_htmlspecialchars($subject); ?>" size="80" maxlength="70" tabindex="<?php echo $cur_index++ ?>" /><br /><br /></label>
<?php endif; ?>			<label class="required"><strong><?php echo $lang_common['Message'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /></label>
						<?php require PUN_ROOT.'mod_modern_bbcode.php'; ?>
						<input type="hidden" value="<?php echo time(); ?>" name="stamp" />
						<textarea id="msg" name="req_message" rows="20" cols="95" tabindex="<?php echo $cur_index++ ?>"><?php echo isset($_POST['req_message']) ? pun_htmlspecialchars($orig_message) : (isset($quote) ? $quote : ''); ?></textarea><br />
						<ul class="bblinks">
							<li><span><a href="help.php#bbcode" onclick="window.open(this.href, 'Help', 'width=800, height=600, resizable=yes, scrollbars=yes'); return false;"><?php echo $lang_common['BBCode'] ?></a> <?php echo ($pun_config['p_message_bbcode'] == '1') ? $lang_common['on'] : $lang_common['off']; ?></span></li>
							<li><span><a href="help.php#img" onclick="window.open(this.href, 'Help', 'width=800, height=600, resizable=yes, scrollbars=yes'); return false;"><?php echo $lang_common['img tag'] ?></a> <?php echo ($pun_config['p_message_bbcode'] == '1' && $pun_config['p_message_img_tag'] == '1') ? $lang_common['on'] : $lang_common['off']; ?></span></li>
							<li><span><a href="help.php#smilies" onclick="window.open(this.href, 'Smilies', 'width=800, height=600, resizable=yes, scrollbars=yes'); return false;"><?php echo $lang_common['Smilies'] ?></a> <?php echo ($pun_config['o_smilies'] == '1') ? $lang_common['on'] : $lang_common['off']; ?></span></li>
						</ul>
					</div>
				</fieldset>
<?php
