...
if (!$pun_user['is_guest'])
	$result = $db->query('SELECT pf.forum_name AS parent_forum, f.parent_forum_id, f.post_inc, f.can_cens, f.forum_name, f.redirect_url, f.moderators, f.num_topics, f.sort_by, fp.post_topics, s.user_id AS is_subscribed FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_subscriptions AS s ON (f.id=s.forum_id AND s.user_id='.$pun_user['id'].') LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') LEFT JOIN '.$db->prefix.'forums AS pf ON f.parent_forum_id=pf.id  WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.id='.$id) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
else
	$result = $db->query('SELECT pf.forum_name AS parent_forum, f.parent_forum_id, f.post_inc, f.can_cens, f.forum_name, f.redirect_url, f.moderators, f.num_topics, f.sort_by, fp.post_topics, 0 AS is_subscribed FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') LEFT JOIN '.$db->prefix.'forums AS pf ON f.parent_forum_id=pf.id  WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.id='.$id) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
...
if (!isset($_GET['p']) || $_GET['p'] == 1)
{
	...
	$subforum_result = $db->query('SELECT f.forum_desc, f.forum_name, f.id, f.last_post, f.last_post_id, f.last_poster, f.moderators, f.num_posts, f.num_topics, f.redirect_url, f.last_topic, us.username, us.id AS last_poster_id FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') LEFT JOIN '.$db->prefix.'users AS us ON (UPPER(us.username)=UPPER(f.last_poster)) WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND parent_forum_id='.$id.' ORDER BY disp_position') or error('Unable to fetch sub forum info',__FILE__,__LINE__,$db->error());
...
<?php
		...
		while ($cur_subforum = $db->fetch_assoc($subforum_result))
		{
			...
			{
				$oldForumTopic = pun_htmlspecialchars($cur_subforum['last_topic']);
				if (pun_strlen($cur_subforum['last_topic']) > $last_title_length)
					$cur_subforum['last_topic'] = utf8_substr($cur_subforum['last_topic'], 0, $last_title_length).'...';

				if ($cur_subforum['last_topic'] != '')
					$last_post = '<div class="avatar_class" style="background-image: url(\''. generate_avatar_markup_raw($cur_subforum['last_poster_id']) .'\');"></div><span class="display-forum-info"><a href="viewtopic.php?pid='.$cur_subforum['last_post_id'].'#p'.$cur_subforum['last_post_id'].'" title="'.$oldForumTopic.'">'.pun_htmlspecialchars($cur_subforum['last_topic']).'</a><br />'.format_time($cur_subforum['last_post']).'<br />'.$lang_common['by'].' <a href="profile.php?id='.$cur_subforum['last_poster_id'].'">'.pun_htmlspecialchars($cur_subforum['last_poster']).'</a></span>';
			}
...
{
	$topic_ids = array();
	for ($i = 0;$cur_topic = $db->fetch_assoc($result, $i);$i++)
