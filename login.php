...
// Require most libraries needed
require PUN_ROOT.'include/recaptcha/vendor/autoload.php';
require PUN_ROOT.'include/openid.php';
require PUN_ROOT.'openid_login.php';
...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	if (!$authorized)
		$errors[] = $lang_login['Wrong user/pass'].' <a href="login.php?action=forget">'.$lang_login['Forgotten pass'].'</a>';

	//if ($cur_user['allow_legacy'] == '0')
	//	$errors[] = 'This user has blocked his account from using the legacy log-in method since it is using the OpenID method.';
...
else if ((isset($_SESSION['key']) && !empty($_SESSION['key'])) && (isset($_SESSION['openid']) && !empty($_SESSION['openid']))) 
{
	$openid_sql_name = '';

	if ($_SESSION['openid'] == 'steam') {
		$openid_sql_name = 'steam64';
	} elseif ($_SESSION['openid'] == 'google') {
		$openid_sql_name = 'google_id';
	}

	if ($openid_sql_name == '')
		redirect('login.php', 'Incorrect OpenID Login, please try again later.');

	$openid_sql = ($db_type == 'mysql' || $db_type == 'mysqli' || $db_type == 'mysql_innodb' || $db_type == 'mysqli_innodb') ? ''.$openid_sql_name.'=\''.$_SESSION['key'].'\'' : 'LOWER('.$openid_sql_name.')=LOWER(\''.$_SESSION['key'].'\')';

	$result = $db->query('SELECT * FROM '.$db->prefix.'users WHERE '.$openid_sql) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	$cur_user = $db->fetch_assoc($result);

	if (empty($cur_user))
		redirect('register.php', 'Redirecting you to finish your account for OpenID Login.');
			...
			<div class="inform">
				<fieldset>
					<legend>reCaptcha</legend>
						<div class="infldset g-recaptcha" data-sitekey="<?php echo CAPTCHA_SITEKEY; ?>"></div>
...
?>
<div id="posterror" class="block">
	<h2><span>Login Request Error</span></h2>
	<div class="box">
		<div class="inbox error-info">
			<p>The following error needs to be corrected before you can be logged in:</p>
			<ul class="error-list">
<?php
		...
		<form id="login" method="post" action="login.php?action=in" onsubmit="return process_form(this)">
			<div class="inform">
				<fieldset>
					...
					<div class="infldset" style="width: 55%; float: left;">
						<input type="hidden" name="form_sent" value="1" />
					...
					<div class="infldset" style="width: 40%; margin-right: 20px; text-align: center; float: right;">
						<div class="login_openid">or you can sign in/register with the OpenID method!</div>
						<center>
