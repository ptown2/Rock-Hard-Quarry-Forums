...
if (isset($_REQUEST['add_ban']) || isset($_GET['edit_ban']))
{
	if (isset($_GET['add_ban']) || isset($_POST['add_ban']))
	{
		...
		{
			if ($user_id == 2)
				message(sprintf('Why are you trying to ban %s? He\'s the fucking owner!', pun_htmlspecialchars($ban_user)));

			if ($user_id == $pun_user['id'])
				message(sprintf('So you\'re trying to ban yourself? Heh...', pun_htmlspecialchars($ban_user)));

			if ($group_id == PUN_ADMIN && $pun_user['id'] != '2')
			...
			if ($is_moderator_group && $pun_user['id'] != '2')
	...
	else // We are editing a ban
	{
		...
		$result = $db->query('SELECT username, ip, email, message, expire, ban_type FROM '.$db->prefix.'bans WHERE id='.$ban_id) or error('Unable to fetch ban info', __FILE__, __LINE__, $db->error());
		if ($db->num_rows($result))
			list($ban_user, $ban_ip, $ban_email, $ban_message, $ban_expire, $ban_type) = $db->fetch_row($result);
		else
			message($lang_common['Bad request']);

		$diff = ($pun_user['timezone'] + $pun_user['dst']) * 3600;
		$ban_expire = ($ban_expire != '') ? $ban_expire - time() : '';
...
<?php endif; ?>				<fieldset>
						...
						<div class="infldset">
							<table class="aligntop" cellspacing="0">
								...
								<tr>
									...
									<td>
										<input type="text" name="ban_expire" size="17" maxlength="10" value="<?php if (isset($ban_expire)) echo ($ban_expire / 60); ?>" tabindex="5" />
										<span><?php echo $lang_admin_bans['Expire date help'] ?></span>
									</td>
								</tr>
								<tr>
									<th scope="row">Ban Type:</th>
									<td>
										<p style="font-size: 12px;">
											<input type="radio" name="ban_type" value="1" tabindex="6" <?php if (isset($ban_type) && $ban_type == '1') echo 'checked'; ?>><?php echo $lang_common['NormBan']; ?></br>
											<input type="radio" name="ban_type" value="2" tabindex="7" <?php if (isset($ban_type) && $ban_type == '2') echo 'checked'; ?>><?php echo $lang_common['SuspBan']; ?></br>
										</p>
										<span>Set which ban is it going to be.</span>
									</td>
								</tr>
							</table>
						</div>
				...
				<p class="submitend"><input type="submit" name="add_edit_ban" value="<?php echo $lang_admin_common['Save'] ?>" tabindex="9" /></p>
			</form>
		</div>
	</div>
...
else if (isset($_POST['add_edit_ban']))
{
	...
	$ban_type = pun_trim($_POST['ban_type']); 
	...
	if ($ban_expire != '' && $ban_expire != 'Never' && $ban_expire != '0')
	{
	...
	$ban_type = ($ban_type != '') ? '\''.$db->escape($ban_type).'\'' : 1;

	if ($_POST['mode'] == 'add')
		$db->query('INSERT INTO '.$db->prefix.'bans (username, ip, email, message, expire, ban_creator, ban_creator_user, ban_type) VALUES('.$ban_user.', '.$ban_ip.', '.$ban_email.', '.$ban_message.', '.$ban_expire.', '.$pun_user['id'].', \''.$pun_user['username'].'\', '.$ban_type.')') or error('Unable to add ban', __FILE__, __LINE__, $db->error());
	else
		$db->query('UPDATE '.$db->prefix.'bans SET username='.$ban_user.', ip='.$ban_ip.', email='.$ban_email.', message='.$ban_message.', expire='.$ban_expire.', ban_type='.$ban_type.' WHERE id='.intval($_POST['ban_id'])) or error('Unable to update ban', __FILE__, __LINE__, $db->error());
		...
		<div class="inbox">
			...
			<thead>
				<tr>
					...
					<th class="tc4" scope="col">Ban Type</th>
...
?>
				<tr>
					...
					<td class="tc4"><?php echo $lang_common[$ban_type_trans[$ban_data['ban_type']]]; ?></td>
